...
- **Patterns and mirrors were built but never took effect** (`freecad_ai/tools/handlers/modifiers.py`). `polar_pattern`, `linear_pattern` and `mirror_feature` created their feature with `body.newObject()` without advancing `body.Tip`, so the Body's shape stayed computed up to the *previous* feature: the pattern appeared in the model tree and in `describe_model`'s feature list while contributing nothing to the geometry. `multi_transform` was the only transformation tool that set the Tip — which is why the same polar step worked through it and failed through `polar_pattern`. Caught in the field: a flange's `polar_pattern(occurrences=8)` reported success with `describe_model` showing 10 faces, the count for a single hole. The Tip is now advanced before the effect is measured, so the check can no longer report a false "NO change" either.

### Changed

- **"Nothing was cut" warnings now say where the shape is** (`freecad_ai/tools/tool_common.py`). The no-material and no-change warnings append the cutting feature's placement next to the body's bounding box — `The cut starts at (62.5, 0.0, 10.0) while the body spans X[-50.0, 50.0] Y[-50.0, 50.0] Z[0.0, 32.0]`. Observed on a real flange: a bolt hole was placed at x=62.5 on a Ø100 disc, so it sat beside the part; the warning fired correctly but its explanation (about local +Z and crossing the face) sent the model hunting the depth instead of the radius, and it burned 25 tool calls before hitting the iteration limit. Wired into `create_primitive` and all three transformation tools.

### Added

- **Patterns and mirrors report whether they changed anything** (`freecad_ai/tools/handlers/modifiers.py`, `freecad_ai/tools/tool_common.py`). `polar_pattern`, `linear_pattern` and `mirror_feature` now measure the body's volume and face count around the feature, recompute inside the transaction, and append the verdict — `(8 occurrences, +14 faces)` when it worked, or a flagged warning when the solid is identical (occurrences coinciding with the original, falling outside the material, or a wrong axis/plane) or when the feature itself ends in an Error/Invalid state. Observed on a real flange: `polar_pattern(occurrences=8)` reported success while producing a single hole — `describe_model` showed 10 faces, the count for one — and the model went on to report eight bolt holes.

- **Open Log / Copy Log buttons in the dock footer** (`freecad_ai/ui/chat_dock/ui.py`, `freecad_ai/ui/chat_dock/session.py`). Next to Save Log: Open Log opens the latest session log in the OS default application (via `QDesktopServices`, so it works the same on Windows/macOS/Linux), and Copy Log puts its contents on the clipboard and reports the size. Both act on the log this session saved, falling back to the newest file in `LOGS_DIR` so they work right after a FreeCAD restart; when there is none they say so instead of failing silently.
...
- **Non-parametric solids are forbidden** (`freecad_ai/tools/handlers/document.py`, `freecad_ai/core/system_prompt.py`, `freecad_ai/config.py`). A `Part::Feature` stores only the final B-Rep — no feature tree, nothing editable afterwards — yet a modelling request rebuilt with raw `Part.makeBox`/`cut()` used to return a plain "success", the dead body only surfacing when the user opened the model tree. `execute_code` now compares the document before and after: any static solid the call introduced is refused, the call is rolled back via `doc.undo()`, and the error names the object and gives the parametric route to redo it. The Act-mode prompt and the tool description state the ban up front so the model doesn't try. Two exemptions: file imports (STEP/STL/mesh — there is no import tool, so `execute_code` is the only route) and an explicit `allow_static_solids` opt-in in `config.json` (JSON-only, defaults to `false`).
